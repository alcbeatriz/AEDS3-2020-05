#include <iostream> //leitura e gravação
#include <locale.h> //acentuação
#include<windows.h> //executar mais lento
using namespace std;
...
bool Verifica_no(No* noAtual)
	{

	if(noAtual != NULL)
		{
		Verifica_no(noAtual->esquerda);
		if(noAtual->esquerda == NULL && noAtual->direita == NULL)
			{
			cout << "O nó " << noAtual->informacao <<
			 " é um nó Folha" << endl;
			}
			else
			{
				Verifica_no(noAtual->direita);
...
// insere as informações: não consegui pegar do teclado :(
	...
	cout << "\n\nApresentação dos nós folhas: " << endl;
	arv.Verifica_no(arv.raiz);
	return 0;
}
